...
| タイヤ位置・カット高さのビジュアライズ | ✅ 実装済み | カット径円柱＋カットゾーン3Dボックス |
| Blender自動検出 | ✅ 実装済み | macOS/Windows/Linux |
| Blender CLIによるSTL生成（フルパイプライン） | ✅ 実装済み | WBスケール→幅高さスケール→中空化→タイヤ除去→底面カット |
...
| カットツールボディの可視化（3D半透明ボックス） | ✅ 実装済み | オレンジ色の除去ゾーン表示 |
| ブーリアン中空化（Solidify廃止） | ✅ 実装済み | 内側縮小コピーとの差分で中空シェル生成 |
...
| カット径（前/後） | Blenderブーリアンで使うシリンダー直径（実際の切り抜きサイズ）。3D表示にも使用。 | mm |
...
1. モデルインポート + 向き調整（transform_apply）
2. メッシュ修復（`bmesh.ops.remove_doubles`）
3. デシメーション（ポリゴン数 > 50K の場合、ratio=0.5）
4. **ホイールベーススケール**（X軸方向: `scale_x = target_wb / current_wb`）
5. **ボディ幅スケール**（Z軸方向: `scale_z = target_width / cur_width`）
6. **ボディ高さスケール**（Y軸方向: `scale_y = target_height / cur_height`）
7. メッシュ修復（スケール後の重複頂点を解消）
8. **中空化**（内側縮小コピーとのブーリアン差分 → 壁厚 ≈ `thickness` mm の中空シェル）
9. **タイヤ除去**（前後左右4本のシリンダーブーリアン差分）
   - 中空化後にタイヤカットすることでホイールハウス内側の余分な壁が生じない
10. **底面カット**（`cut_z` 以下をボックスブーリアン差分で切り落とす）
    - `cut_z` はピック時のY座標（スケール前基準）。Y方向スケール係数で補正して適用。
11. 中間ファイル保存（`preview/intermediate.blend`）
12. loose parts分離 → `preview/loose_parts.json` 出力
13. 最大体積オブジェクトを `preview/result.stl` として出力

#### タイヤカットのみ（mode="tire_cut_only"）

ステップ1〜9（中空化＋タイヤ除去まで）を実行して出力。底面カットはスキップ。
パラメータ調整のテストに使う。

#### 中空化アルゴリズム（`_hollow_boolean()`）

```python
# 各軸の縮小スケール = (外寸 - 2*thickness) / 外寸
sx = (cur_x - 2 * thickness) / cur_x
sy = (cur_y - 2 * thickness) / cur_y
sz = (cur_z - 2 * thickness) / cur_z

# バウンディングボックス中心に原点移動 → スケール適用 → ブーリアン差分
bpy.ops.object.duplicate(linked=False)          # 内側コピーを作成
bpy.ops.object.origin_set(type='ORIGIN_GEOMETRY', center='BOUNDS')
inner_obj.scale *= (sx, sy, sz)                 # 中心まわりに均等縮小
_boolean_subtract(outer_obj, inner_obj)         # 外側 - 内側 = 中空シェル
```

平坦面での壁厚は `thickness` mm に正確に一致する。

#### 処理完了後の表示

- `preview/result.stl` をビューポートにロード → **テール色（緑がかった青）のオーバーレイ**で表示
- 元モデルは非表示（結果のみ表示）
- **カット径の黄色い円柱**を表示（スケーリング前の入力値で描画）
- **カットゾーンのオレンジ色3Dボックス**をビューポートに表示（除去された底部の領域）
- 「✕ 結果オーバーレイをクリア」ボタンが出現
...
- `bmesh.calc_volume()` の戻り値は数値的に mm³（座標がmmのため）
...
- 結果ロード時: 元モデルを非表示、テール色（teal: `rgba(0.0, 0.88, 0.72, 0.95)`）の結果メッシュを表示
- カット径の黄色い円柱を常に表示（スケーリング前の入力値で描画）
- カットゾーンをオレンジ色の半透明3Dボックスで表示（除去領域の可視化）
  - 結果表示中: 結果モデルのBBoxとスケール後のcut_z（`cut_z * scale_y`）を使用
  - 通常時: 元モデルのBBoxと入力cut_zを使用
  - cut_z がモデル底面以下の場合: 赤色の警告平面と警告テキストを表示
